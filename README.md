...
> `stock_state.json`（在庫状態の記録）は初回実行時にリポジトリの初期ファイルを使用し、
> 以降は GitHub Actions Cache で実行間に引き継がれます（git commit は不要）。
...
> ⚠️ ワークフロー一覧の「**Disable**」ボタンは関係ありません。以下の手順で操作してください。

1. リポジトリの「**Actions**」タブを開く
2. 左側のワークフロー一覧から「**監視 一時停止 / 再開**」を選択
3. 右側の「**Run workflow**」ボタンをクリック
4. チェックボックスで操作を選択して「**Run workflow**」を実行：
   - チェックをオンにして実行 → **一時停止**（在庫チェックをスキップ）
   - チェックをオフにして実行 → **再開**（通常通り動作）

内部的には GitHub Actions のリポジトリ変数 `STOCK_CHECK_PAUSED` を更新します。`check_stock.yml` はこの変数が `true` の場合にジョブをスキップします。
